...
public class SearchServiceImpl implements SearchService {
    ...
    public PageResult<SearchResult> search(SearchRequest request) {
        log.info("执行搜索: keywords={}, page={}, size={}, sortField={}, sortDirection={}", 
        ...
        try {
            ...
            if (pagedResults.isEmpty()) {
                ...
                log.debug("搜索结果第一条: {}", pagedResults.get(0));
                log.debug("搜索结果ID列表: {}", 
                         pagedResults.stream().map(SearchResult::getId).collect(Collectors.toList()));
            }
    ...
    public long count(String keywords) {
        log.info("开始计算匹配结果数量: keywords={}", keywords);
    ...
    private String extractContentFromXml(String xmlContent) {
        ...
        try {
            ...
            if (nodesCount > 0) {
                log.info("找到 <Content> 标签 {} 个", nodesCount);
                ...
                if (!result.isEmpty()) {
                    log.info("成功从 <Content> 标签提取内容，长度: {}, 内容前缀: {}", 
                            result.length(), 
            ...
            log.error("解析XML内容失败: {}, XML前100个字符: {}", 
                    e.getMessage(), 
                    xmlContent.length() > 100 ? xmlContent.substring(0, 100) + "..." : xmlContent, 
                    e);
            
            // 尝试使用正则表达式提取Content标签内容
            try {
                log.info("尝试正则表达式提取Content标签内容");
                ...
                while (matcher.find()) {
                    ...
                    log.info("正则表达式匹配到第{}个 <Content> 标签", count);
                ...
                if (count > 0) {
                    ...
                    log.info("正则表达式匹配到{}个Content标签，提取结果长度: {}", count, result.length());
